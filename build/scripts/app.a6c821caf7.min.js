...
var authWindow,DOMAIN="axel.org",FACEBOOK_APP_ID="axel.org"==DOMAIN?"178551732274362":"686285104834353",GOOGLE_APP_ID="659914067642-3sak3lnsnq8rv24qgrp2jg6s3ga3r7e4.apps.googleusercontent.com";function initSignin(){jQuery("input[type=email]").prop({pattern:"^\\s*([A-Za-z0-9!#$%&*+/\\-^=?_{|}~]+[.])*[A-Za-z0-9!#$%&*+/\\-^=?_{|}~]+@(([A-Za-z0-9]+[.-])*[A-Za-z0-9]+\\.[A-Za-z]{2,})\\s*$"}).on("input",validateEmail),jQuery(".f_login_btn").on("click",openFacebookAuth),jQuery(".g_login_btn").on("click",openGoogleAuth),jQuery(window).on("message",function(e){"setAuthCookie"==e.originalEvent.data.command&&onLoginSuccess()})}function login(e){var t=e.email.value.toLowerCase();return jQuery.post("https://online."+DOMAIN+"/opus/user",{_a:"get_auth_types",email:t}).always(function(e){e&&e.code&&"0"==e.code?!e.data.length||-1<e.data.indexOf("stoamigo")||-1<e.data.indexOf("stoamigo_without_password")||-1<e.data.indexOf("gccas")?(location.href="https://login."+DOMAIN+"/?email="+encodeURIComponent(t)+"&auth_types="+encodeURIComponent(JSON.stringify(e.data)),ga("gtm1.send",{hitType:"event",eventCategory:"New User",eventAction:"Signs Up",eventLabel:"using Signup for Free Button"})):-1<e.data.indexOf("facebook")?openFacebookAuth():-1<e.data.indexOf("google")&&openGoogleAuth():alert("Failed to sign in.")}),!1}function validateEmail(){this.validity.patternMismatch?this.setCustomValidity("Email has invalid format"):this.setCustomValidity("")}function openFacebookAuth(){ga("gtm1.send",{hitType:"event",eventCategory:"New User",eventAction:"Signs Up",eventLabel:"using Facebook Sign in Button"}),openWindow("https://www.facebook.com/dialog/oauth?client_id="+FACEBOOK_APP_ID+"&scope=email,publish_actions&redirect_uri="+encodeURIComponent("https://static."+DOMAIN+"/?auth=facebook&app=web"))}function openGoogleAuth(){ga("gtm1.send",{hitType:"event",eventCategory:"New User",eventAction:"Signs Up",eventLabel:"using Google Sign in Button"}),openWindow("https://accounts.google.com/o/oauth2/auth?client_id="+GOOGLE_APP_ID+"&scope=profile+email&response_type=code&redirect_uri="+encodeURIComponent("https://online."+DOMAIN+"/?auth=google&app=web"))}function openWindow(e){var t,n=Math.round((screen.availHeight-600)/2),o=Math.round((screen.availWidth-800)/2),i=window.navigator.userAgent;authWindow&&!authWindow.closed?authWindow.location.href=e:authWindow=window.open(e,"authWindow","toolbar=0,menubar=0,status=0,width=800,height=600,top="+(n<0?0:n)+",left="+(o<0?0:o)),authWindow.focus(),(-1<i.indexOf("MSIE ")||/Trident.*rv\:11\./.test(i))&&(t=window.setInterval(function(){authWindow&&!authWindow.closed||(window.clearInterval(t),onLoginSuccess())},100))}function onLoginSuccess(){location.href="https://online."+DOMAIN+"/"}if(jQuery(initSignin),void 0===ga)function ga(){}
...
!function(a){a.fn.text3d=function(t){var e=a.extend({},{depth:5,angle:100,color:"#ddd",lighten:-.15,shadowDepth:10,shadowAngle:80,shadowOpacity:.3},t);function o(t,a){(t=String(t).replace(/[^0-9a-f]/gi,"")).length<6&&(t=t[0]+t[0]+t[1]+t[1]+t[2]+t[2]),a=a||0;var n,h,e="#";for(h=0;h<3;h++)n=parseInt(t.substr(2*h,2),16),e+=("00"+(n=Math.round(Math.min(Math.max(0,n+n*a),255)).toString(16))).substr(n.length);return e}return e.angle=e.angle*Math.PI/180,e.shadowAngle=e.shadowAngle*Math.PI/180,this.each(function(){!function(t){var a,n,h="";for(a=1;a<=e.depth;a++)h+=Math.round(Math.cos(e.angle)*a)+"px "+Math.round(Math.sin(e.angle)*a)+"px 0 "+o(e.color,(a-1)/(e.depth-1)*e.lighten)+",";for(a=n=1;n<=e.shadowDepth;a++)n+=a,h+=Math.round(Math.cos(e.shadowAngle)*n)+"px "+Math.round(Math.sin(e.shadowAngle)*n)+"px "+n+"px rgba(0,0,0,"+(e.shadowOpacity-(e.shadowOpacity-.1)*a/e.shadowDepth)+"),";t.style.textShadow=h.substr(0,h.length-1)}(this)}),this}}(jQuery),jQuery(function(){jQuery(".text3d").text3d()});
